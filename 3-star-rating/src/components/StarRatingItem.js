...
export default class StarRatingItem extends Component {
  ...
  getFillPattern(value) {
    const compareValue = this.$props.hoverValue > 0 ? this.$props.hoverValue : this.$props.ratingValue;
    if (value <= compareValue) return STAR_FILL_VALUE.FULL;
    if (value === Math.round(compareValue)) return STAR_FILL_VALUE.HALF;
    if (value > compareValue) return STAR_FILL_VALUE.EMPTY;
  }
  ...
  template() {
    return `<path
    id="star"
    fill="${this.getFillPattern(this.$props.value)}"
  ...
  handleMouseMove({ target, clientX }) {
    const ratingValue = this.$props.value;
    ...
    if (mouseX >= halfWidth) {
      ...
      this.$props.setHoverValue(+ratingValue - 0.5);
    }
  }
